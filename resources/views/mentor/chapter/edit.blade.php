...
@section('content')
    ...
    @endif
    <form action="{{ route('mentor.update.chapter', $chapter->id) }}" method="POST" enctype="multipart/form-data" >
        @csrf
        @method('PUT')
        <div class="form-row">
            <div class="col-md-12 mb-3">
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <select name="course_id" class="form-select" >
                        @foreach ($course as $c)
                        <option value="{{$c->id}}" {{ $c->id == $chapter->course_id ? 'selected' : ''  }}  >{{$c->title}}</option>
                        @endforeach
                    </select>

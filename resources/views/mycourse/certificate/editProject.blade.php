    ...
    <div class="container" data-aos="fade-up">
      ...
      <div class="row mt-5">
        ...
        <div class="col-lg-12 mt-5 mt-lg-0">
          ...
          <form action="{{ route('update.data.Project',$data->id) }}" method="POST" enctype="multipart/form-data" >
            @csrf
            @method('PUT')
            <div class="row">
            ...
            <div class="form-group mt-3">
                ...
                <select name="mycourse_id" class="form-select" >
                    @foreach ($course as $c)
                    <option value="{{$c->mycourse_id}}">{{$c->title}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mt-3 ">
                <label for="">Link Project :</label>
                <input type="text" class="form-control" name="link_project" value="{{$c->link_project}}" id="link" placeholder="Input Link Project" >
            </div>
            <div class="form-group mt-3 ">
              <label for="">Description:</label>
              <textarea name="description" id="" class="form-control" cols="30" rows="10">{{$c->description}}</textarea>
          </div>
            <div class="form-group mt-3">
                <label>Image 1 :</label>
